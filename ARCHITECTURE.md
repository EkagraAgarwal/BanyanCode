...
| `RepositoryIntelligence` (Wave 2) | 8 active public methods: `query`, `explain`, `impact`, `trace`, `tests`, `symbols`, `relationships`, `ownership` (`slice` retired). All slice-returning methods emit the stable `ArchitecturalSlice` shape. Workspace context propagated via optional `workspace?: WorkspaceContext`. Internal helpers (`findSymbol`, `findSubsystem`, `findEntrypoints`, `findTests`, `findRelated`, `findCallers`, `findDependencies`, `walkSubsystem`) stay in the layer and back a shared `bfsPure` primitive in `bfs.ts`. Tests discovery is evidence-based: `tested_by` edges first, then `calls`/`references` from test → symbol (batched), then `import` substring match as last-resort low-confidence fallback. Focus-dir misses surface an `outside-focus-dirs` diagnostic; no silent fallback to out-of-scope nodes. | `CodegraphRepo`, `Git` |
...
- `POST /global/codegraph-remove` — clear the index (`dropFile: true` unlinks the DB file too)
...
- **Wave 2** (`main`): repository intelligence v2. Reshaped `RepositoryIntelligence` from 7 to a stable **8-method public surface** (`query`, `explain`, `impact`, `trace`, `tests`, `symbols`, `relationships`, `ownership`). `slice` was retired and `explain` is the canonical slice-returning method. New canonical return type `ArchitecturalSlice` (`{ summary, entrypoints, importantSymbols, relatedTests, relatedDocs, configs, routes, dependencies }`). `Search` gained `searchAuto(query, opts)` cascade + `mode: "manual"` SDK escape. `StructuralQueries` added `findInterfaces`, `findExports`, `findImports`. Codegraph `CodegraphNodeKind` extended with `ci | docker | env | doc`; new parsers in `langs/{markdown,docker}.ts`. Shared types in `packages/core/src/banyancode/types.ts`: `ArchitecturalSlice`, `RepositoryContext`, `WorkspaceContext`, `Ranking`, `Diagnostic`. New `Banyan.Git` service (`packages/core/src/banyancode/repository-intelligence/git-service.ts`) wrapping `Banyan.$` over the bundled git binary.
- **Wave 3 / V3.1 review fixes**: refactored traversal onto a shared `bfsPure` primitive (`packages/core/src/banyancode/repository-intelligence/bfs.ts`) with per-direction edge-kind allowlists, head-index queue (no `Array.shift`), and batched `edgesFromBatch`/`edgesToBatch` queries (2 queries per frontier instead of 2 per node). Evidence-based test discovery ranks `tested_by` > `calls`/`references` > `import` substring. `outside-focus-dirs` diagnostic replaces silent fallback. Indexer lifecycle: producer + drain now run concurrently in `indexFiles()` (fixes 129-file deadlock), tree-sitter tree cache capped at 1000 entries with LRU eviction and `removeFiles()` cleanup, query grammar sources cached at layer construction. SQL migration `20260719000000_codegraph_node_name_idx` adds a standalone name index. HTTP route `POST /global/codegraph-remove` is now wired (was advertised but missing).
  ...
  - **Tools**: 8 active `repository_*` tool wrappers in `packages/core/src/tool/repository-wave2.ts` (`repository_query`, `repository_explain`, `repository_impact`, `repository_trace`, `repository_tests`, `repository_symbols`, `repository_relationships`, `repository_ownership`). `repository_slice` was retired and superseded by `repository_explain`. Each passes through `PermissionV2.assert(...)` and `traced(...)`, returns the schema-validated output, and is registered via `banyanToolLayers` in `packages/opencode/src/tool/registry.ts`.
  ...
  - **Trace**: `TraceEvent` has optional `cache?: { hit: boolean; key?: string }` and `workspace?: WorkspaceContext` slots. Trace file is bounded to **7 days OR 10,000 events**, whichever first; oldest lines dropped from `.banyancode/trace/<sessionID>.jsonl`. CLI subcommand `opencode codegraph trace --session <id> [--limit N]` tails the file. The `intel-trace-panel.tsx` TUI sidebar widget is authored but not registered — left as a Wave-4 task.
