...
describe("session.llm.ai-sdk adapter", () => {
  ...
  // A4 / #34022: OpenAI-compatible providers surface cached tokens as
  // `usage.prompt_tokens_details.cached_tokens`, which the AI SDK maps to
  // `inputTokenDetails.cacheReadTokens` (v6 shape) or the legacy top-level
  // `cachedInputTokens` (v5 shape). Both must survive into LLMEvent usage —
  // including the finish event's totalUsage, which the session cost accounting
  // reads — or the whole input gets billed at the fresh rate (cache_read
  // reported 0 despite the provider serving cached tokens).
  test("preserves OpenAI-compatible cache tokens from both AI SDK usage shapes through the finish event", async () => {
    const events = await adapt([
      {
        type: "finish-step",
        response: { id: "r1", timestamp: new Date(0), modelId: "muse-spark" },
        finishReason: "stop",
        rawFinishReason: "stop",
        providerMetadata: undefined,
        // @ai-sdk/openai-compatible v6 shape (from prompt_tokens_details.cached_tokens).
        usage: {
          inputTokens: 1000,
          outputTokens: 500,
          totalTokens: 1500,
          inputTokenDetails: { noCacheTokens: 700, cacheReadTokens: 300, cacheWriteTokens: undefined },
          outputTokenDetails: { textTokens: 500, reasoningTokens: undefined },
        },
      },
      {
        ...
        finishReason: "stop",
        rawFinishReason: "stop",
        // Legacy v5 shape: no inputTokenDetails, cachedInputTokens directly.
        totalUsage: {
          inputTokens: 2000,
          outputTokens: 500,
          totalTokens: 2500,
          cachedInputTokens: 800,
          inputTokenDetails: { noCacheTokens: 1200, cacheReadTokens: undefined, cacheWriteTokens: undefined },
          outputTokenDetails: { textTokens: 500, reasoningTokens: undefined },
        },
      },
    ])

    const stepFinish = events.find((event) => event.type === "step-finish")
    expect(stepFinish?.usage?.cacheReadInputTokens).toBe(300)

    const finish = events.find((event) => event.type === "finish")
    if (!finish?.usage) throw new Error("expected finish usage")
    expect(finish.usage.cacheReadInputTokens).toBe(800)

    // End-to-end: getUsage bills finish-event cache.read at the cache rate —
    // Meta Muse contributor pricing ($0.10/M fresh, $0.002/M cached).
    const result = SessionNs.getUsage({
      model: {
        id: "muse-spark",
        providerID: "meta",
        name: "Muse Spark",
        limit: { context: 200_000, output: 8_000 },
        cost: { input: 0.1, output: 0.2, cache: { read: 0.002, write: 0 } },
        capabilities: {
          toolcall: true,
          attachment: false,
          reasoning: true,
    ...
    expect(result.tokens.cache.read).toBe(800)
    expect(result.tokens.input).toBe(1200)
    expect(result.cost).toBeCloseTo((1200 * 0.1 + 500 * 0.2 + 800 * 0.002) / 1_000_000, 12)
  })
