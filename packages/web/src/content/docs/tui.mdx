...
Configure the model's reasoning mode (on/off) or set its reasoning effort/thinking level (using standard OpenAI or Anthropic values).

Accepts values like `low`, `medium`, `high`, `xhigh`, `max`, `on` (enables reasoning with a default/high level), and `off` / `none` / `disabled` (disables reasoning). If run without arguments, it prints the current active variant and all available variants for the active model.
...
:::note
These commands configure the model's reasoning level/budget for the current session. To cycle through model variants using a keybind, you can also press `ctrl+t`.
:::
