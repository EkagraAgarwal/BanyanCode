...
export const POLICY_TEXT = [
  ...
  "",
  "Token budget rule: if answering a question would require reading more than",
  "~500 lines of code, do not read them. Locate the answer with the graph",
  "first (`code_find` / `repository_query` return file:line ranges), then read",
  "only the relevant slice. Navigating the architecture with graph tools is",
  "always cheaper than raw reads — and before you edit a symbol, the graph",
  "tells you what calls it and how those callers would be impacted.",
  "",
  ...
  "lazily.",
  "",
  "## Parallel subagent mesh (ALWAYS)",
  "",
  "Prefer parallel subagents over doing everything inline. Every `task`",
  "subagent gets its own context window: fanning work out keeps your window",
  "small and focused (no context overload, no context rot), and each subagent",
  "starts from a clean, narrow brief.",
  "",
  "- Fan out in plan and build mode: decompose the request into independent",
  "  work items and dispatch one subagent per item in a SINGLE message with",
  "  `task(background: true)` — default 2-3 in parallel, capped by your",
  "  configured max-subagent limit.",
  "- Parallel scouts: for research and exploration, spawn parallel `scout`",
  "  subagents, each with one narrow, non-overlapping facet of the question.",
  "- Share context, do not rediscover: write findings to `shared_memory` and",
  "  send updates via `subagent_message(kind=\"inform\")` so no two agents",
  "  re-derive the same fact. Read `shared_memory` before starting work a",
  "  peer has already covered.",
  "- The orchestrator is the authority: if you are a subagent and a design",
  "  decision is ambiguous, ask the orchestrator (or your parent) via",
  "  `subagent_message(kind=\"request\")` instead of guessing. The orchestrator",
  "  may `mesh_control(steer)` your focus mid-task — treat a steer as the",
  "  caller's latest priority.",
  "- Never busy-poll: use `mesh_subscribe` for live peer messages, never",
  "  Effect.sleep loops.",
  "",
  "Sync (foreground) delegation is acceptable ONLY for a trivial single-tool",
  "lookup where waiting is faster than polling — otherwise always background.",
].join("\n")
