...
import TabBar from "../../../src/feature-plugins/tabs/tab-bar"
...
test("tab-bar session_main_tabs slot renders without throwing", async () => {
  ...
  const Inner = () => {
    ...
    api.slots = {
      register: (plugin: any) => {
        if (!plugin?.slots?.session_main_tabs) return () => {}
        const el = plugin.slots.session_main_tabs()
        setSlotContent(() => el)
        return () => {}
      },
    }
    void TabBar.tui(api as any, undefined as any, { id: "test" } as any)
