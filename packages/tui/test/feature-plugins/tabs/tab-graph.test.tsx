...
import TabGraph from "../../../src/feature-plugins/tabs/tab-graph"
...
test("tab-graph session_tab_graph slot renders without throwing", async () => {
  ...
  const Inner = () => {
    ...
    api.slots = {
      register: (plugin: any) => {
        if (!plugin?.slots?.session_tab_graph) return () => {}
        const el = plugin.slots.session_tab_graph()
        setSlotContent(() => el)
        return () => {}
      },
    }
    void TabGraph.tui(api as any, undefined as any, { id: "test" } as any)
