...
import TabSettings from "../../../src/feature-plugins/tabs/tab-settings"
...
test("tab-settings session_tab_settings slot renders without throwing", async () => {
  ...
  const Inner = () => {
    ...
    api.slots = {
      register: (plugin: any) => {
        if (!plugin?.slots?.session_tab_settings) return () => {}
        const el = plugin.slots.session_tab_settings()
        setSlotContent(() => el)
        return () => {}
      },
    }
    void TabSettings.tui(api as any, undefined as any, { id: "test" } as any)
